...
WORK = ROOT / "xiangyu_real_clips_v2_work"
...
SOURCES = [
    ...
    ("battle", "https://www.bilibili.com/video/BV1viw2eQEiG/", "项羽二十八骑突围片段"),
    ("ending", "https://www.bilibili.com/video/BV1AVP1eiEWh/", "项羽乌江自刎片段"),
]
FALLBACKS = [
    ("hongmen", "https://tv.cctv.com/2019/10/27/VIDEBVriZOYCAQ6wdb96jraN191027.shtml", "央视纪录片《楚汉》鸿门之宴"),
    ("gaixia", "https://tv.cctv.com/2019/10/29/VIDEMH6B7FZSjpaJFQtoHaEi191029.shtml", "央视纪录片《楚汉》霸王别姬"),
]
...
def download(name: str, url: str) -> Path | None:
    ...
    run(cmd, check=False, timeout=210)
    ...
    for candidate in candidates:
        ...
        run([
            ...
            "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart", str(converted),
        ], timeout=180)
        return converted
    return None
...
def collect() -> tuple[list[Path], list[str]]:
    ...
    if len(clips) < 2:
        for name, url, label in FALLBACKS:
            clip = download(name, url)
            if clip:
                clips.append(clip)
                credits.append(f"{label} | {url}")
            if len(clips) >= 3:
                break
    if len(clips) < 2:
        raise RuntimeError("少于两段可用的项羽相关视频")
...
async def voice() -> tuple[Path, list[tuple[float, float, str]]]:
    ...
    gap = VOICE / "gap.mp3"
    run(["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "0.08", "-q:a", "4", str(gap)])
    gap_d = duration(gap)
    parts: list[Path] = []
    ...
    run([
        ...
        "-af", "acompressor=threshold=-20dB:ratio=3.5:attack=3:release=70,"
               "equalizer=f=140:t=q:w=1.1:g=3,"
               "equalizer=f=2800:t=q:w=1.0:g=2.2,"
               "loudnorm=I=-14:LRA=4:TP=-1.2",
...
Style: Sub,Noto Sans CJK SC,58,&H00FFFFFF,&H00FFFFFF,&H000A0A0A,&H72000000,-1,0,0,0,100,100,0,0,3,2,0,2,55,55,125,1
Style: Title,Noto Sans CJK SC,82,&H00FFFFFF,&H00FFFFFF,&H00101010,&H52000000,-1,0,0,0,100,100,0,0,1,5,2,8,55,55,135,1
...
"""
    ...
    events.append(
        f"Dialogue: 7,{ass_time(max(0,total-2.2))},{ass_time(total)},End,,0,0,0,,"
...
def segment(src: Path, dest: Path, start: float, length: float, idx: int) -> None:
    d = duration(src)
    start = 0.0 if d <= length + 0.1 else max(0.0, min(start, d - length - 0.05))
    bias = [0.50, 0.43, 0.57, 0.47, 0.53][idx % 5]
    vf = (
        "scale=1080:1920:force_original_aspect_ratio=increase,"
        f"crop=1080:1920:(iw-ow)*{bias}:(ih-oh)/2,"
        "fps=30,setsar=1,eq=contrast=1.08:saturation=0.95:brightness=-0.015,"
        "unsharp=5:5:0.3:3:3:0.12,vignette=PI/9,format=yuv420p"
    )
    run([
...
def render(clips: list[Path], narration: Path, timings: list[tuple[float, float, str]], credits: list[str]) -> tuple[Path, Path]:
    ...
    for i in range(count):
        source_i = i % len(clips)
        usable = max(0.0, durations[source_i] - seg_len - 0.1)
        start = ((i // len(clips)) * 6.4 + source_i * 4.1) % usable if usable > 0 else 0.0
        part = WORK / f"segment_{i:02d}.mp4"
        segment(clips[source_i], part, start, seg_len, i)
        parts.append(part)
    ...
    final = OUT / "项羽为什么输给刘邦_真实片段快语速版.mp4"
    fc = (
        f"[0:v]subtitles={ass.as_posix()}:fontsdir={FONT_DIR}[v];"
        f"sine=frequency=60:duration={total:.3f}:sample_rate=48000,tremolo=f=2.1:d=0.84,volume=0.022[beat];"
        f"anoisesrc=color=brown:amplitude=0.004:duration={total:.3f},lowpass=f=220[air];"
