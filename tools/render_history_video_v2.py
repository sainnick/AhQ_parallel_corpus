...
WORK = ROOT / "history_render_work_v2"
CLIPS = WORK / "clips"
VOICE = WORK / "voice"
OUT = ROOT / "out"
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/126 Safari/537.36"

# Direct downloadable files verified before rendering.
SOURCES = [
    ...
    ("xian_terracotta", "https://cdn.pixabay.com/video/2021/07/07/80606-572349661_large.mp4"),
    ("chinese_gate", "https://cdn.pixabay.com/video/2021/05/10/73743-549547469_large.mp4"),
    ("sage_statue", "https://cdn.pixabay.com/video/2023/10/06/183843-872156884_large.mp4"),
    ("ancient_general", "https://cdn.pixabay.com/video/2024/09/10/230724_large.mp4"),
    ("warrior", "https://cdn.pixabay.com/video/2023/05/13/162949-826740711_large.mp4"),
]
...
def download(url: str, dest: Path) -> None:
    ...
    for attempt in range(4):
        try:
            with SESSION.get(url, stream=True, timeout=(25, 240), allow_redirects=True) as r:
                r.raise_for_status()
...
Style: Sub,Noto Sans CJK SC,54,&H00FFFFFF,&H00FFFFFF,&H00101010,&H33000000,-1,0,0,0,100,100,0,0,1,4,1,2,70,70,170,1
Style: Title,Noto Sans CJK SC,78,&H00FFFFFF,&H00FFFFFF,&H00202020,&H22000000,-1,0,0,0,100,100,1,0,1,5,2,5,70,70,0,1
Style: End,Noto Sans CJK SC,61,&H00FFFFFF,&H00FFFFFF,&H00202020,&H22000000,-1,0,0,0,100,100,1,0,1,4,1,8,70,70,210,1

[Events]
Format: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text
"""
    events: list[str] = []
    title_end = min(timings[0][1] + 0.2, 4.8)
    events.append(
...
def prepare_segment(src: Path, dest: Path, length: float, index: int) -> None:
    ...
    start = (index * 1.31) % room if room > 0.2 else 0.0
    vf = (
        "scale=1080:1920:force_original_aspect_ratio=increase,"
        "crop=1080:1920:(iw-ow)/2:(ih-oh)/2,"
        "fps=30,setsar=1,"
        "eq=contrast=1.08:saturation=0.82:brightness=-0.02,"
...
def render(clips: list[Path], narration: Path, timings: list[tuple[float, float, str]]) -> Path:
    ...
    # Start wide, then alternate action, relics, architecture and atmosphere.
    order = [2, 0, 1, 4, 7, 3, 0, 5, 1, 8, 4, 2, 6, 0, 3, 5, 1, 4, 2, 6]
    segments: list[Path] = []
